...
pub fn run_datagen(options: DatagenOptions) {
    ...
    if options.games % options.threads != 0 {
        println!(
            "{ORANGE}WARNING: {DEFAULT}The number of games is not divisible by the number of threads!"
        );
    }
...
fn gen_random_move(pos: &Position) -> Option<Move> {
    ...
    let mut m = Move::NULL;
    for _ in 0..8 {
        m = move_list.moves[fastrand::usize(..move_list.len())];
...
fn datagen_thread(id: usize, games: usize, tc: TimeControl, tb: TB, path: &Path) {
    ...
    'main: for games_played in 0..games {
        ...
        for _ in 0..fastrand::usize(8..=9) {
            if let Some(m) = gen_random_move(&position) {
        ...
        let game_result = loop {
            ...
            if best_move == Move::NULL {
                println!(
                    "{ORANGE}WARNING: {DEFAULT}The engine returned a null move in this position: {}",
                    &position.board
                );
            }
