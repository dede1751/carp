...
impl TTFlag {
    /// Checks if the flag and the value triple cause a score cutoff.
    pub fn cutoff(self, value: Eval, alpha: Eval, beta: Eval) -> bool {
        self == TTFlag::Exact
            || (self == TTFlag::Lower && value >= beta)
            || (self == TTFlag::Upper && value <= alpha)
    }
}
